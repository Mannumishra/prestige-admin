...
const AddProduct = () => {
  ...
  const filterSubcategories = () => {
    // console.log(JSON.stringify(selectedMainCategory))
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    try {
      let res = await axios.post("http://localhost:8000/api/product", formData)
      console.log(res);
    } catch (error) {
      console.log(error);
    }
        ...
        <div className="container">
          <form className="row" onSubmit={handleSubmit}>
            ...
            <div className="col-md-3">
              ...
              <input type="text" className="form-control" name='productname' onChange={handleChange} placeholder="Product Name" aria-label="Product Name" />
            </div>
            <div className="col-md-3">
              <label htmlFor="productPrice" className="form-label">businesstype</label>
              <input type="text" className="form-control" name='businesstype' onChange={handleChange} placeholder="businesstype" aria-label="Product Price" />
            </div>
            <div className="col-md-3">
              <label htmlFor="length" className="form-label">material</label>
              <input type="text" className="form-control" name='material' onChange={handleChange} placeholder="Length" aria-label="Length" />
            </div>

            <div className="col-md-3">
              <label htmlFor="width" className="form-label">length</label>
              <input type="text" className="form-control" name='length' onChange={handleChange} placeholder="length" aria-label="Width" />
            </div>

            <div className="col-md-3">
              <label htmlFor="thickness" className="form-label">volteage</label>
              <input type="text" className="form-control" name='volteage' onChange={handleChange} placeholder="volteage" aria-label="Thickness" />
            </div>

            <div className="col-md-3">
              <label htmlFor="size" className="form-label">application</label>
              <input type="text" className="form-control" name='application' onChange={handleChange} placeholder="application" aria-label="Size" />
            </div>
